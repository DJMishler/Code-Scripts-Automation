...
END) [RunStatus], 
mda.subscriber_db [SubscriberDB], 
mda.publication [PUBName],
...
--CONVERT(VARCHAR(500),mdh.[time]) [LastSynchronized],
und.UndelivCmdsInDistDB [UndistCom], 
mdh.comments [Comments], 
'select * from distribution.dbo.msrepl_errors (nolock) where id = ' + CAST(mdh.error_id AS VARCHAR(8)) [QueryMoreInfo],
...
mda.publisher_db+' - '+CAST(mda.publisher_database_id as varchar) [PublisherDB],
...
--and mdh.runstatus IN ('2', '5', '6') --Succeed(2), Retry(5), Fail(6)
and mdh.runstatus = '4'
order by mdh.[time]
